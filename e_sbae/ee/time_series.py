...
from ..ee.landsat import landsat_collection
from ..helpers.ee import (
    export_to_ee, delete_sub_folder, processing_grid, _ee_export_table, cleanup_tmp_esbae
)
from ..helpers.python import timer
...
def extract(input_grid, config_dict, start_res=8, end_res=13):
    ...
    if config_file.exists():
        ...
        keys_list = ["work_dir", "workers", "max_points_per_chunk", "grid_size_levels"]
        ...
        if not new_config_dict == old_config_dict:
            config_change = input(
                ...
                "If you continue, all of your already processed files will be deleted. "
...
def cascaded_extraction_ee(input_grid, config_file):
    ...
    for i in range(0, size, 25000):
        ...
        if size > 25000:
            logger.info(f"------------------------------------------------")
            logger.info(f"Processing subset {i/25000+1}/{subsets}")
            logger.info(f"------------------------------------------------")

        sub = ee.FeatureCollection(input_grid.toList(25000, i))
        for chunk_size in chunk_sizes:

            _ = _parallel_extract_ee(sub, chunk_size, config_dict, subset=i/25000+1)
            logger.info("Checking for points not processed at thecurrent aggregation level.")
    ...
    logger.info(
        "Cleaning up temporary Earth Engine assets created during the processing."
    )
    cleanup_tmp_esbae()
    ...
    if len(list(final_dir.glob('*geojson'))) > 0:
        ...
        if isinstance(input_grid, gpd.geodataframe.GeoDataFrame):
            ...
            if input_grid.size().getInfo() > 0:
                logger.warning(
                    "Points within those files are discarded and a "
                    "temporary FeatureCollection including the non-processed points "
...
def _parallel_extract_ee(points_fc, chunk_size, config_dict, subset):
    ...
    # automatically get aoi from extent of points and upload it as a fixed asset
    logger.info(
    ...
    logger.info(
        f"Creating processing chunks of {chunk_size} degrees for parallel extraction."
    )
    chunks_fc = processing_grid(aoi_fc, chunk_size)
    processing_chunks = ee.FeatureCollection(chunks_fc).aggregate_array(
        '.geo').getInfo()
    ...
    for i, chunk in enumerate(processing_chunks):
        ...
        nr_points = cell_fc.size().getInfo()

        if 0 < nr_points < config_dict['max_points_per_chunk']:
    ...
    logger.info(
        f"Sending {len(args_to_process)} to the parallel extraction routine."
    )
    # run in parallel
    if args_to_process:
        return_code = _run_in_threads(extract_time_series, args_to_process, config_dict)
    else:
        return_code = 1
...
def _run_in_threads(func, arg_list, config_dict):
    ...
    with concurrent.futures.ThreadPoolExecutor(config_dict["workers"]) as executor:
...
def structure_ts_data(df, point_id_name, bands):
    ...
    for i, point in enumerate(df[point_id_name].unique()):
        ...
        d[i] = {
            # "point_idx": i,  # as we have the point id, this should not be needed
            point_id_name: point,
