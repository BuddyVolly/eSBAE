...
def squared_grid(aoi, spacing, crs="ESRI:54017", sampling_strategy="systematic"):
    ...
    if not aoi.crs:
        crs_original = input(
            "Your AOI does not have a coordinate refernce system (CRS). Please provide the CRS of the AOI (e.g. epsg:4326): "
        )
...
def hexagonal_gridold(
    ...
    grid = dggrid_instance.grid_cell_polygons_for_extent(
        projection, resolution, clip_geom=aoi.dissolve().geometry.values[0]
    )
...
def hexagonal_grid(
    ...
    # run dggrid
    return_code = run_command(f"dggrid {tmp_meta}")
...
def upload_to_ee(gdf, asset_name):
    ...
    if len(gdf) > 25000:
        print(
            "Need to run splitted upload routine as dataframe has more than 25000 rows"
        )
