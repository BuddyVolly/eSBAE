...
def get_aoi_areas(aoi, collection, start=2001, end=2022, tree_cover=20, mmu=0.5):
    ...
    span = end - start + 1
    print(
        f"According to the GFC product, the Area of Interest covers an area of {d['total_area']} square kilometers,"
...
def calculate_error_cochran(total_area, subarea, z_score=1.645, sample_size=250):
    ...
    return (
        np.sqrt(
            (z_score * z_score * proportional_change * (1 - proportional_change))
            / sample_size
        )
        / proportional_change
    )
...
def run_cochran(area_dict, max_error=10):
    ...
    for idx, sample_size in enumerate(range(10000, 500000, 10000)):
        change_error = (
            calculate_error_cochran(
                total_area=area_dict["total_area"],
    ...
    df.columns = [
        "Sample Size",
        "Theoretical Sampling Error (Forest)",
        "Theoretical Sampling Error (Deforestation)",
        "Grid Size",
    ]
...
def get_sampling_errors(
    ...
    if collection == "CCI":
        ...
        change = hansen.gte(ee.Number(start).subtract(2000)).And(
            hansen.lte(ee.Number(end).subtract(2000))
        )
        scale = 30
    ...
    def get_grid_sample_error(grid):
        ...
        def get_sampled_proportional_change(seed, proj):
            ...
            points = (
                random.eq(maximum).selfMask().clip(aoi).reproject(proj.atScale(scale))
            )

            # create a stack with change and total pixels as 1
            stack = (
                change.updateMask(points)  # masked sample change
            ...
            proportional_change_sampled = (
                ee.Number(areas.get("sampled_change"))
                .divide(ee.Number(areas.get("sampled_area")))
                .getInfo()
            )
            print(proportional_change_sampled)
            error = (
                (np.abs(proportional_change_sampled - proportional_change))
                / proportional_change
                * 100
            )
            return error
        ...
        sampling_iter = np.array(
            [get_sampled_proportional_change(seed, proj) for seed in seeds]
        )
    ...
    print(
        f" Running the sampling error simulation. Please be patient, this can take a while."
    )
    for idx, grid in enumerate(grid_sizes):
        print(
            f" Running {nr_of_runs_per_grid} times the sample error simulation at a scale of {grid} meter."
        )
...
# we should invert that to do something similar as extrating from global data
def calculate_sample_size_cochran(
    total_area, subarea, z_score=1.645, target_precision=0.2
):

    proportional_change = subarea / total_area
    error = target_precision * proportional_change
    return (z_score * z_score * proportional_change * (1 - proportional_change)) / (
        error * error
    )
