...
from ..helpers.python import split_dataframe
...
def line_from_coords(coordinates, from_index, to_index):
    return ee.Geometry.LineString(
        ee.List([coordinates.get(from_index), coordinates.get(to_index)])
    )
...
def value(_list, index):
    return ee.Number(_list.get(index))
...
def where(condition, true_value, false_value):
    true_masked = true_value.mask(condition)
    false_masked = false_value.mask(invert_mask(condition))
    return true_masked.unmask(false_masked)
...
def determine_footprint(image):
    ...
    def find_corner(target_value, values):
        diff = values.map(lambda _value: ee.Number(_value).subtract(target_value).abs())
    ...
    return ee.List([upper_left, lower_left, lower_right, upper_right, upper_left])
